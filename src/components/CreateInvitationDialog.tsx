...
export function CreateInvitationDialog({ isOpen, onClose }: CreateInvitationDialogProps) {
  ...
  const invitationLink = createdInvitation
    ? `${window.location.origin}/invitation/${createdInvitation.token}`
    : '';
          ...
          {createdInvitation ? (
            <div className="space-y-5">
              <div className="text-center py-4">
                ...
                <p className="text-foreground mb-2" style={{ fontFamily: 'Anton, sans-serif', fontSize: '1.2rem' }}>
                  Lien d'invitation prêt !
                </p>
                <p className="text-muted-foreground text-sm">
                  Partagez ce lien à la personne que vous souhaitez inviter.
                </p>
              </div>

              <div className="bg-background border border-border p-4">
                <div className="flex items-center justify-between gap-2">
                  <code className="text-xs text-muted-foreground truncate flex-1" style={{ fontFamily: 'JetBrains Mono, monospace' }}>
                    {invitationLink}
                  </code>
                  <button
                    onClick={handleCopy}
              ...
              <div className="text-xs text-muted-foreground space-y-1">
                <p>Expire le : {new Date(createdInvitation.expireLe).toLocaleDateString('fr-FR', { day: '2-digit', month: 'long', year: 'numeric' })}</p>
                <p>Usages : {createdInvitation.usagesCount}{createdInvitation.maxUsages !== null ? ` / ${createdInvitation.maxUsages}` : ' / ∞'}</p>
              </div>
            </div>
          ) : (
            <div className="space-y-5">
              ...
              <div>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    ...
                    disabled={isSubmitting}
                  />
                  <span className="text-sm font-medium">Limiter le nombre d'inscriptions</span>
                </label>
                {maxUsagesEnabled && (
                  <div className="mt-3">
                    <input
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Le lien sera désactivé après {maxUsages} inscription{maxUsages > 1 ? 's' : ''}.
                    </p>
                  </div>
                )}
                {!maxUsagesEnabled && (
              ...
              <div className="bg-primary/5 border border-primary/20 p-4 text-xs text-muted-foreground">
                <p>
                  Le lien généré permettra à un slaméur de remplir un formulaire public pour
                  rejoindre votre collectif en tant que membre.
                </p>
              </div>
            </div>
          )}
